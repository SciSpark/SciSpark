...
object MCSUtils {
	 ...
	 */
	def netcdf_write(filename:String, varData:ucar.ma2.ArrayInt.D2, lats:Array[Double], lons:Array[Double]): Unit = {
	 ...
	 * 
	 */
	def get_node_data(edgeArray:Array[(String, String)], MCSNodeMap:mutable.HashMap[String, MCCNode], lats:Array[Double], lons:Array[Double], tightestBox:Boolean ): Unit = {
		edgeArray.map({ e => 
	 ...
	 * 
	 */
	def get_all_node_data(edgeArray:Array[(String, String)], MCSNodeMap:mutable.HashMap[String, MCCNode], lats:Array[Double], lons:Array[Double], tightestBox:Boolean ): Unit = {
		edgeArray.map({ e => 
	...
	/** Extract the node mask from the MCCNode metadata for the 
	...
	def extract_masked_data(thisNode: MCCNode, lats:Array[Double], lons:Array[Double], tightestBox:Boolean): Unit = {
        ...
        nodeID = "/tmp/F"+thisNode.getFrameNum.toString+"CE"+thisNode.getCloudElemNum.toString.dropRight(2)+"_"+latMinOffset+"_"+latMaxOffset+"_"+lonMinOffset+"_"+lonMaxOffset+".nc"
