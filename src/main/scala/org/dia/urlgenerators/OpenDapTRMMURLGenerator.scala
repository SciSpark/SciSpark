...
object OpenDapTRMMURLGenerator {
  ...
  val URL = "http://disc2.nascom.nasa.gov/opendap/hyrax/TRMM_L3/"
  // "http://disc2.nascom.nasa.gov:80/opendap/TRMM_L3/"
   ...
   */
  def numOfDaysAndDate(currYear: Int, startTime: String, endTime: String) : (Int, Int, DateTime) = {
    ...
    for (year <- numYears) {
      ...
      for (day <- 1 to days) {
        ...
        for (hr <- numHrs) {
          ...
          if (hr != 0) {
            paddedDay = ((startday + day).toString.reverse + "00").substring(0, 3).reverse
          }
          val sb = new StringBuilder()
