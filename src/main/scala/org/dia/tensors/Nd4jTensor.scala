...
class Nd4jTensor(t : (Any) => (Array[Double], Array[Int])) extends AbstractTensor {
   ...
   * @return
   */
  def loadNetCDFTRMMVars(url: String, variable: String): Nd4jTensor = {
   ...
   * @return
   */
  def loadNetCDFNDVars(url: String, variable: String): Nd4jTensor =  {
    val netcdfFile = NetCDFUtils.loadNetCDFDataSet(url)
   ...
   * @return
   */
  def loadNetCDFNDVars(netcdfFile: NetcdfDataset, variable: String): Nd4jTensor = {
    val coordinateArray = NetCDFUtils.convertMa2ArrayTo1DJavaArray(netcdfFile, variable)
    val dims = NetCDFUtils.getDimensionSizes(netcdfFile, variable)
    val shape = dims.toArray.sortBy(_._1).map(_._2)
    new Nd4jTensor(Nd4j.create(coordinateArray, shape))
  }
   ...
   * @return DenseMatrix
   */
  def create2dArray(dimensionSizes: mutable.HashMap[Int, Int], netcdfFile: NetcdfDataset, variable: String): Nd4jTensor = {
    ...
    new Nd4jTensor(Nd4j.create(coordinateArray, Array(x, y)))
  }
