...
/**
 * Utilities for working with WWLN data in SciSpark 
 */

object WWLNUtils extends Serializable {

  val logger = org.slf4j.LoggerFactory.getLogger(this.getClass)

  /**
   * Creates a column of datetime string in SimpleDateFormat("yyyy/MM/DD kk:mm:ss") 
   * from the first two columns of WWLN data
   ...
   */
  def getWWLNtimeStr(datePart: String, timePart: String): String = {
   ...
   */
  def getWWLNDateTime(currTimeStr: String): Timestamp = {
  ...
  /**
   * Queries the WWLN data for lightning strike at a given time within a region-box
   * @param WWLN The broadcasted WWLN data stored within a dataframe
   ...
   * @return Results of the query to an Array of tuples in the format (Float, Float). If value
   *		 is given as Array() then no lightning strikes were found. 
   */
  def getLightningLocs(WWLN: Broadcast[DataFrame],
                     thisTimeDate: String, 
                     minLat: Double,
                     maxLat: Double,
                     minLon: Double,
    ...
    val result = WWLN.value.filter(filterCmd)
